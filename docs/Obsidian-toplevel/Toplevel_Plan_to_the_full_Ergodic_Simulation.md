...
- **User (The Human):** The actual person logging in.
    ...
    - **UI:** We 4 label the field **"Workspace / Organization Name"**. We will add a tip: _"Students/Individuals: Enter 'My Workspace' or your name."_
...
- Have an example company and find, cannot be edited by the user, but can be copied. Our have as suggested the version that can be shared visible on the landing page then logon and a copy is immediately on your page to play with. 
...
- Standard shocks (free version) (Company+Investor functionality)
	- Unspecific shock. 
	- Marketing
	- Staffing. Random time 2-7 months boring time, hiring cost x months salary Google normal. And then add a dampening effect on growth and revenue.
