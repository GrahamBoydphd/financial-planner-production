...
<file path='frontend/app/fund-plan/[fundPlanId]/results/page.tsx'>
'use client';
...
export default function FundResultsPage({ params }: { params: { fundPlanId: string } }) {
  ...
  // 1. Initial Load (Fund Metadata & Plan Config)
  useEffect(() => {
    const load = async () => {
      try {
        // Fetch Plan first
        ...
        <div className="flex flex-wrap items-center gap-4 bg-white p-3 rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="flex flex-col gap-1">
                ...
                >
                  ...
                  <option value="monte_carlo">Equally likely real world fan</option>
                </select>
              </div>
              ...
              {viewMode !== 'standard' && (
                <>
                    ...
                    <div className="flex flex-col gap-1 w-40 animate-in fade-in slide-in-from-left-2 duration-300">
                            ...
                            onChange={(e) => setPoolingFraction(parseInt(e.target.value))}
                            onMouseUp={handleSavePooling}
                            onTouchEnd={handleSavePooling}
