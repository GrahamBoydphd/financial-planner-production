...
apiClient.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      // Common fix: Remove wrapping quotes if the token was stored using JSON.stringify
...
export const api = {
  ...
  createFund: async (name: string, currency_code: string = 'USD') => 
    (await apiClient.post<Fund>('/api/funds', { name, currency_code })).data,
  updateFund: async (id: string, name: string, currency_code?: string) => 
    (await apiClient.put<Fund>(`/api/funds/${id}`, { name, currency_code })).data,
  ...
  createCompany: async (name: string, fund_id: string, industry?: string, business_model?: string, technology?: string, currency_code: string = 'USD') => 
    (await apiClient.post<Company>('/api/companies', { name, fund_id, industry, business_model, technology, currency_code })).data,
  updateCompany: async (id: string, updates: Partial<Company>) => 
    (await apiClient.put<Company>(`/api/companies/${id}`, updates)).data,
