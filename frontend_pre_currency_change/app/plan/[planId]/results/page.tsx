...
import { api, FinancialPlan, CapitalInjection, DividendPolicy, CreditFacility, ValuationAssumption, getCurrencySymbol } from '@/lib/api';
...
export default function ResultsPage({ params }: { params: { planId: string } }) {
  ...
  // Derived UI Settings
  const currencyCode = plan?.company?.currency_code || plan?.company?.fund?.currency_code || 'USD';
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Fetch Currency from Fund if missing
        try {
            if (!p.company?.currency_code && !p.company?.fund?.currency_code) {
                const c = await api.getCompany(p.company_id);
                const f = await api.getFund(c.fund_id);
                // Attach to plan object for consistency
                p = { ...p, company: { ...c, fund: f } };
            }
        } catch (err) {
            console.error("Failed to fetch fund currency", err);
        }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            <Card>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input type="number" placeholder={`Amount (${currency})`} className="border p-1 text-xs rounded" 
