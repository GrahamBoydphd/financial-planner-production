...
import { StaffingRole, api } from "@/lib/api"
...
export default function StaffingForm({ planId, initialRoles, onSave, onDelete }: StaffingFormProps) {
  ...
  // --- EFFECT: FETCH CURRENCY ---
  useEffect(() => {
    let active = true;
    const fetchCurrency = async () => {
        try {
            const p = await api.getPlan(planId);
            if (p.company?.fund?.currency_symbol) {
                if (active) setCurrency(p.company.fund.currency_symbol);
                return;
            }
            const c = await api.getCompany(p.company_id);
            const f = await api.getFund(c.fund_id);
            if (active) setCurrency(f.currency_symbol || '$');
        } catch (e) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label htmlFor="annual_salary" className="block text-sm font-medium text-gray-700 flex items-center gap-2">
                  Annual Salary ({currency}) <span className="text-red-500">*</span>
