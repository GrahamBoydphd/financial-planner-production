...
pub async fn create_plan(
    ...
    let plan = sqlx::query_as!(
        FinancialPlan,
        "INSERT INTO financial_plans (company_id, name, start_month, currency_code, tenant_id) VALUES ($1, $2, $3, $4, $5) RETURNING id, company_id, tenant_id, name, start_month, currency_code, initial_cash, pooling_fraction, created_at, updated_at",
        payload.company_id,
        payload.name,
...
pub async fn update_plan(
    ...
    let plan: Option<FinancialPlan> = sqlx::query_as!(
        ...
        "UPDATE financial_plans 
         ...
         RETURNING id, company_id, tenant_id, name, start_month, currency_code, initial_cash, pooling_fraction, created_at, updated_at",
        payload.name,
        start_date,
...
pub async fn get_all_plans(
    ...
    let plans = sqlx::query_as!(
        FinancialPlan,
        "SELECT id, company_id, tenant_id, name, start_month, currency_code, initial_cash, pooling_fraction, created_at, updated_at FROM financial_plans WHERE tenant_id = $1 ORDER BY created_at DESC",
        claims.tenant_id
    )
    .fetch_all(&pool)
    .await?;
...
pub async fn get_plan(
    ...
    let plan: Option<FinancialPlan> = sqlx::query_as!(
        FinancialPlan,
        "SELECT id, company_id, tenant_id, name, start_month, currency_code, initial_cash, pooling_fraction, created_at, updated_at FROM financial_plans WHERE id = $1 AND tenant_id = $2",
        id,
        claims.tenant_id
    )
    .fetch_optional(&pool)
