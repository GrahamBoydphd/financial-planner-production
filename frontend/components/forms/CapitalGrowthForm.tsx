...
export default function CapitalGrowthForm({ planId, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    api.getCapitalGrowth(planId).then(p => {
      ...
      // Default to Simple Mode unless user switches or logic dictates otherwise
    }).catch(() => {});
    return () => { active = false; };
  }, [planId]);
        ...
        {volType !== 'none' && (
            <div className="bg-indigo-50 p-3 rounded text-sm space-y-3">
                 ...
                 <div>
                    ...
                    <input type="number" step="0.01" className="w-full border p-1" value={mean} onChange={e => setMean(e.target.value)} placeholder="e.g. 0.5" />
                 </div>
                 ...
                 {!isAdvanced && volType === 'nrig' && (
                     <div className="space-y-3">
                         <div>
                             ...
                             <select className="w-full border p-1 rounded text-xs" value={alpha} onChange={e => setAlpha(e.target.value)}>
                         ...
                         <div>
                             ...
                             <select className="w-full border p-1 rounded text-xs" value={beta} onChange={e => setBeta(e.target.value)}>
                                 <option value="">-- Select --</option>
                                 {BETA_OPTIONS.map(o => (
                                     <option key={o.value} value={o.value}>{o.label}</option>
                                 ))}
                         ...
                         <div>
                             ...
                             <select className="w-full border p-1 rounded text-xs" value={scale} onChange={e => setScale(e.target.value)}>
                                 <option value="">-- Select --</option>
                                 {SCALE_OPTIONS.map(o => (
                                     <option key={o.value} value={o.value}>{o.label}</option>
                                 ))}
                        ...
                        {volType === 'flat' && (
                            <div className="grid grid-cols-3 gap-2">
                                <div>
                                    ...
                                    <input className="w-full border p-1" value={volMin} onChange={e => setVolMin(e.target.value)} />
                                </div>
                                <div>
                                    <label className="text-xs text-gray-400">Max %</label>
                                    <input className="w-full border p-1" value={volMax} onChange={e => setVolMax(e.target.value)} />
                                </div>
                                <div>
                                    <label className="text-xs text-gray-400">Steps</label>
                                    <input className="w-full border p-1" value={volIntervals} onChange={e => setVolIntervals(e.target.value)} />
                                </div>
                            </div>
                        )}
                        ...
                        {volType === 'student_t' && (
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input className="w-full border p-1" value={scale} onChange={e => setScale(e.target.value)} placeholder="e.g. 1.0" />
                                </div>
                                <div>
                                    <label className="text-xs text-gray-400">Freedom (Deg)</label>
                                    <input className="w-full border p-1" value={freedom} onChange={e => setFreedom(e.target.value)} placeholder="e.g. 5.0" />
                                </div>
                            </div>
                        )}
                        ...
                        {volType === 'nrig' && (
                            <div className="grid grid-cols-3 gap-2">
                                <div>
                                    ...
                                    <input className="w-full border p-1" value={alpha} onChange={e => setAlpha(e.target.value)} placeholder="e.g. 1.0" />
                                </div>
                                <div>
                                    <label className="text-xs text-gray-400">Imbalance (Beta)</label>
                                    <input className="w-full border p-1" value={beta} onChange={e => setBeta(e.target.value)} placeholder="e.g. 0.0" />
                                </div>
                                <div>
